...
export const CustomDomain = ({ onClick }: Props) => {
	const [t] = useT()
	...
	const handleOnClick = () => {
		if (workspace.subscription === 'free') {
			...
			Modals.domainConfiguration.open()
			onClick()
		}
	}
			...
			<Container>
				...
				<ConfigureContainer>
					{project?.preferences?.customDomain && (
						<>
							{t(`You currently have a custom domain link to this project`, 'customDomainPresent')}
							<ButtonGroup>
								<LinkCard link={project.preferences.customDomain} />
								<SettingButton onClick={handleOnClick}>
									<EditIcon size={14} color={theme.colors.dark} />
								</SettingButton>
								<SettingButton
									onClick={() => {
										Modals.deleteDomainConfirmation.open({ project })
										onClick()
									}}
								>
					...
					{!project?.preferences?.customDomain && (
						<>
							...
							<SettingButton onClick={handleOnClick} primary>
