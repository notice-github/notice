...
export const OnboardingForm = ({ onFinish }: Props) => {
	const [t] = useT()
	const user = useUser()
	...
	const onSkip = () => {
		updateUser.mutate({ formIsFilled: true })
		onFinish()
	}
		...
		<Container>
			...
			<FormContainer gap={26}>
				...
				<Row gap={18}>
					...
					>
						<StyledColumn gap={4}>
							<FormLabel>{t('Phone Number', 'phoneNumber')}</FormLabel>
							<PhoneInputWrapper>
								<PhoneInput
			...
			<Footer>
				...
				<SkipButton onClick={onSkip}>{t('Skip', 'skip')}</SkipButton>
			</Footer>
		</Container>
	)
}
